...
Deno.serve(async (req) => {
    try {
        ...
        const response = await fetch(`${SUNO_API_BASE}/midi/generate`, {
            method: 'POST',
            headers: {
        ...
        if (data.code !== 200) {
            ...
            return Response.json({ 
                error: data.msg || 'MIDI generation failed',
                details: data
            }, { status: 400 });
        }
        ...
        console.error('Error in generateMidi:', error);
        return Response.json({ 
            error: error.message || 'Failed to generate MIDI'
        }, { status: 500 });
    }
});